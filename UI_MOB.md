...
- **PoD** (`proof_of_delivery_page.dart`): **shipper** + **người nhận** cùng ký (`OrgSignaturePad` → multipart `shipperSignature` + `signature`) + ảnh PoD (`file`) + tên người nhận; BE sinh **PDF biên bản bàn giao** (hai chữ ký) → `handoverDocumentUrl`, lưu cloud; sau gửi mở `shipper_handover_pdf_page.dart` (pdfx). Chỉ khi `in_transit`.
...
| **Xác nhận giao, ký biên bản & PoD** | [proof_of_delivery_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/shipper/presentation/pages/proof_of_delivery_page.dart)<br>[shipper_handover_pdf_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/shipper/presentation/pages/shipper_handover_pdf_page.dart) | [shipper_remote_datasource.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/shipper/data/datasources/shipper_remote_datasource.dart)<br>[shipper_repository.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/shipper/data/repositories/shipper_repository.dart) | `POST /api/v1/shipper/deliveries/{id}/proof` → `handoverDocumentUrl` | ✅ Hoàn thành (Gọi API thực) |
...
| **Đặt suất ăn tập trung (chọn option)** | [org_bulk_order_mode_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/organization/presentation/pages/org_bulk_order_mode_page.dart) *(2 option: **Tự động** / **Theo hợp đồng**)* | — | Điều hướng tới 2 flow bên dưới | ✅ Hoàn thành |
| **Đặt suất ăn theo hợp đồng (Period-Based)** | [org_meal_period_contract_index_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/organization/presentation/pages/org_meal_period_contract_index_page.dart) *(4 bước khớp web Index: Kỳ HĐ + lịch loại trừ/suất ngày → Giao hàng → KM → Tạo nháp)*<br>[org_meal_period_contract_review_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/organization/presentation/pages/org_meal_period_contract_review_page.dart) *(Rà soát → checkout → ký phụ lục → PayOS)*<br>[org_meal_period_weekly_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/organization/presentation/pages/org_meal_period_weekly_page.dart) *(Chọn món tuần — Weekly)*<br>[org_order_annex_sign_page.dart](file:///h:/EngineeringThesis/Khoa_Luan_KS_Mobile/lib/src/features/organization/presentation/pages/org_order_annex_sign_page.dart) *(ký phụ lục; PayOS qua **meal-contract-order**)* | `GET /api/v1/organization/meal-contract-order/portion-prices`<br>`GET /api/v1/organization/meal-contract-order/dish/main`<br>`POST /api/v1/organization/meal-contract-order/contract`<br>`POST /api/v1/organization/meal-contract-order/checkout`<br>`POST /api/v1/organization/meal-contract-order/pay`<br>`POST /api/v1/organization/meal-contract-order/{contractId}/weekly-meals` | ✅ Hoàn thành (core flow) |
...
| Xác nhận giao + ký biên bản + PoD | POST | `/api/v1/shipper/deliveries/{id}/proof` | Form-data: `file` (ảnh PoD), **`shipperSignature`** (PNG chữ ký shipper), **`signature`** (PNG chữ ký người nhận), **`RecipientConfirmedName`**, `notes` (optional). Đơn `in_transit` → `completed`. BE sinh PDF biên bản (hai bên ký), lưu cloud. Response: `shipperSignatureUrl`, `recipientSignatureUrl`, **`handoverDocumentUrl`**. | `roles:Shipper` + `deliveries.update` |
...
└── shipper/
    ...
    └── presentation/
        ...
        │   ├── shipper_handover_pdf_page.dart
        │   ├── route_map_page.dart
