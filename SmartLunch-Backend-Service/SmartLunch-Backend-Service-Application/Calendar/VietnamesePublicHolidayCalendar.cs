namespace SmartLunch.Backend.Service.Application.Calendar;
...
{
    public sealed record HolidayInfo(DateOnly Date, string Name, string Kind);
    ...
    /// <summary>Ngày lễ âm lịch / di động theo từng năm (dd/MM/yyyy).</summary>
    ...
    public static IReadOnlyList<HolidayInfo> GetHolidays(DateOnly from, DateOnly to)
    {
        ...
        var map = new Dictionary<DateOnly, HolidayInfo>();

        for (var y = from.Year; y <= to.Year; y++)
        {
            foreach (var (month, day, name) in FixedSolar)
            {
                TryAdd(map, new DateOnly(y, month, day), name, "fixed");
            }
    ...
    public static string? GetLabel(DateOnly date)
    {
        var list = GetHolidays(date, date);
        return list.Count == 0 ? null : list[0].Name;
    }

    public static string? GetWeekendLabel(DateOnly date) =>
    ...
    private static void TryAdd(Dictionary<DateOnly, HolidayInfo> map, DateOnly date, string name, string kind)
    {
        if (!map.ContainsKey(date))
            map[date] = new HolidayInfo(date, name, kind);
        else if (!map[date].Name.Contains(name, StringComparison.Ordinal))
            map[date] = map[date] with { Name = map[date].Name + " · " + name };
    }
}
