CREATE TABLE orders (
    Id INT NOT NULL AUTO_INCREMENT,
    Code VARCHAR(20) NULL COMMENT 'Mã tự sinh (trigger)',
    UserId INT NULL,
    ContractId INT NULL COMMENT 'Hợp đồng liên quan',
    OrderDate DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ScheduledDate DATE NOT NULL,
    Status VARCHAR(20) NOT NULL DEFAULT 'pending',
    TotalAmount DECIMAL(12,2) NOT NULL,
    SubtotalAmount DECIMAL(12,2) NULL COMMENT 'Tổng trước KM',
    DiscountAmount DECIMAL(12,2) NOT NULL DEFAULT 0 COMMENT 'Tiền giảm',
    PaymentStatus VARCHAR(20) NOT NULL DEFAULT 'unpaid',
    InvoiceCode VARCHAR(40) NULL,
    AnnexPdfUrl VARCHAR(2048) NULL COMMENT 'URL signed PDF phụ lục đơn',
    AnnexSignedAt DATETIME NULL COMMENT 'Thời điểm ký phụ lục (UTC)',
    RecipientName VARCHAR(120) NULL COMMENT 'Tên người nhận',
    RecipientPhone VARCHAR(20) NULL COMMENT 'SĐT người nhận',
    RecipientEmail VARCHAR(254) NULL COMMENT 'Email nhận thông báo',
    DeliveryAddress VARCHAR(500) NULL COMMENT 'Địa chỉ giao hàng',
    DeliveryWardDistrict VARCHAR(255) NULL COMMENT 'Phường/xã, quận/huyện',
    DeliveryNotes VARCHAR(500) NULL COMMENT 'Ghi chú giao hàng',
    PreferredDeliveryTime VARCHAR(32) NULL COMMENT 'Giờ giao mong muốn (HH:mm)',
    OrderConfirmationEmailSentAt DATETIME NULL COMMENT 'Đã gửi email xác nhận đơn',
    PaymentReminderSentAt DATETIME NULL COMMENT 'Đã gửi email nhắc thanh toán',
    CreatedBySalesUserId INT NULL,
    CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UpdatedAt DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (Id),
    UNIQUE KEY UK_orders_code (Code),
    UNIQUE INDEX UQ_orders_invoice_code (InvoiceCode),
    INDEX IX_orders_user (UserId),
    INDEX IX_orders_contract (ContractId),
    INDEX IX_orders_scheduled_status (ScheduledDate, Status),
    INDEX IX_orders_recipient_email (RecipientEmail),
    INDEX IX_orders_payment_reminder (PaymentStatus, PaymentReminderSentAt),
    CONSTRAINT FK_orders_user FOREIGN KEY (UserId) REFERENCES users (Id) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT FK_orders_contract FOREIGN KEY (ContractId) REFERENCES contracts (Id) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT FK_orders_sales FOREIGN KEY (CreatedBySalesUserId) REFERENCES users (Id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci ROW_FORMAT = DYNAMIC COMMENT = 'Đơn hàng suất ăn';
