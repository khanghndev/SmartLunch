...
SELECT (SELECT Id FROM users WHERE Username = 'customer1'), (SELECT Id FROM dishes WHERE Name = 'Cơm sườn nướng'), @ord3, 5, 'Cơm sườn nướng rất ngon, thịt mềm gia vị vừa ăn!';
INSERT INTO reviews (UserId, DishId, OrderId, Rating, Comment)
SELECT (SELECT Id FROM users WHERE Username = 'customer2'), (SELECT Id FROM dishes WHERE Name = 'Cơm gà chiên mắm'), @ord4, 4, 'Gà chiên mắm ngon, hơi mặn. Cần giảm nước mắm.';
INSERT INTO reviews (UserId, DishId, OrderId, Rating, Comment)
SELECT (SELECT Id FROM users WHERE Username = 'abc_company'), (SELECT Id FROM dishes WHERE Name = 'Cơm chiên dương châu'), @ord1, 5, 'Cơm chiên dương châu rất ngon, nhân viên công ty rất thích!';
...
SELECT (SELECT Id FROM users WHERE Username = 'xyz_school'), @ord2, 'Giao hàng trễ', 'Đơn hàng cho trường XYZ bị giao trễ 30 phút.', 'in_progress', (SELECT Id FROM users WHERE Username = 'admin'), NULL;
INSERT INTO complaints (UserId, OrderId, Title, Description, Status, AssignedTo)
SELECT (SELECT Id FROM users WHERE Username = 'customer1'), @ord3, 'Thiếu món', 'Đặt 2 món nhưng chỉ nhận 1. Thiếu canh chua tôm.', 'new', NULL;

-- Chatbot logs
INSERT INTO chatbot_logs (UserId, Message, Response) VALUES
((SELECT Id FROM users WHERE Username = 'customer1'), 'Hôm nay có món gì ăn trưa?', 'Hôm nay thực đơn trưa gồm: Cơm sườn nướng, Rau muống xào tỏi, Canh chua tôm.'),
((SELECT Id FROM users WHERE Username = 'customer2'), 'Tôi muốn xem đánh giá món cơm gà', 'Cơm gà chiên mắm có đánh giá trung bình 4/5 sao.'),
