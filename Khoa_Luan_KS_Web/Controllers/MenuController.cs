...
namespace Khoa_Luan_KS_Web.Controllers
{
    public class MenuController : Controller
    {
            ...
            CancellationToken ct = default)
        {
            ...
            {
                await LoadRelatedDishesAsync(vm, id, ct);
            }

            return View(vm);
        }

        private async Task LoadRelatedDishesAsync(DishDetailPageViewModel vm, int dishId, CancellationToken ct)
        {
            var dish = vm.Detail?.Dish;
            if (dish == null)
                return;

            var slotKey = dish.PrimarySlotKey?.Trim();
            ...
            try
            {
                ...
                var cat = categories.Categories.FirstOrDefault(c =>
                    string.Equals(c.SlotKey, slotKey, StringComparison.OrdinalIgnoreCase));
                if (cat == null)
                    return;

                vm.RelatedCategoryId = cat.Id;
                var browse = await _masterDataClient.GetPublicDishesBrowseAsync(1, 12, cat.Id, null, ct);
                vm.RelatedDishes = browse.Items
                    .Where(i => i.Id != dishId)
                    .Take(4)
                    .ToList();
            }
